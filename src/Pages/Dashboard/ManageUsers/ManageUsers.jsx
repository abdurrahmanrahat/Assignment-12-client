...
const ManageUsers = () => {
  ...
  const handleMakeAdmin = (user) => {
    if (user.role === "admin") {
      ...
      fetch(
        `https://assignment-12-server-lyart-xi.vercel.app/users/admin/${user._id}`,
        {
          method: "PATCH",
        }
      )
  ...
  const handleMakeInstructor = (user) => {
    ...
    if (user.role === "instructor") {
      ...
      fetch(
        `https://assignment-12-server-lyart-xi.vercel.app/users/instructor/${user._id}`,
        {
          method: "PATCH",
        }
      )
          ...
          <tbody>
            {users?.map((user, index) => (
              ...
              >
                ...
                <td>{user.name}</td>
                <td>{user.email}</td>
                <td className="text-lg">{user.role ? user.role : "student"}</td>
                <td>
                  <button
                    onClick={() => handleMakeInstructor(user)}
                    className="btn btn-ghost btn-sm bg-[#F75DC8] hover:bg-[#0E0C1A] text-white hover:text-white"
                  >
                    Instructor
                  </button>
                </td>
